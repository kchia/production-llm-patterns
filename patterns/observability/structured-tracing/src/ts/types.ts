 ...
 *
 * Core types for the tracing system: spans, traces, exporters, and LLM-specific attributes.
 */
...
export interface TracerConfig {
  /** Where traces go — console, HTTP endpoint, or custom backend */
  ...
  /** How often to batch-export spans (ms). Default: 5000 */
  flushIntervalMs?: number;

  /** Max spans queued before dropping. Default: 2048 */
  maxQueueSize?: number;

  /** Service name for identifying the source. Default: 'llm-service' */
  serviceName?: string;
}

export const DEFAULT_TRACER_CONFIG: Required<TracerConfig> = {
  exporter: { export: async () => {}, shutdown: async () => {} },
...
// --- LLM Request/Response types (for mock provider) ---
...
export class SpanDroppedError extends TracingError {
  constructor(spanName: string, reason: string) {
    super(`Span "${spanName}" dropped: ${reason}`, 'SPAN_DROPPED');
    this.name = 'SpanDroppedError';
  }
}
