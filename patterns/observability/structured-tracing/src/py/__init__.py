"""Structured Tracing — Core Implementation (Python).

A lightweight, framework-agnostic tracer for LLM pipelines.
Uses contextvars for automatic span nesting, keeping the API clean
...
AsyncLocalStorage (which doesn't propagate through setTimeout).
"""
...
class ConsoleExporter:
    """Prints spans as JSON to stdout."""
...
_active_entry: ContextVar[_SpanStackEntry | None] = ContextVar(
    "_active_entry", default=None
)
...
class Tracer:
    ...
    def trace_llm_call(
        ...
        """Trace an LLM call with standard attributes:
    ...
    def _start_trace(
        ...
        """Begin a root trace, returning (span, context_token)."""
        ...
        span = Span(name, SpanContext(trace_id=trace_id, span_id=span_id), self._max_span_attributes)
    ...
    def _start_child_span(
        ...
        """Begin a child span, returning (span, context_token, parent_entry)."""
...
class _TraceContextManager:
    """Context manager returned by tracer.trace()."""
...
class _RetrievalContextManager:
    ...
    def set_result(
        self, document_count: int, top_score: float | None = None,
    ) -> None:
