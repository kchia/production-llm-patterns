"""Prompt Version Registry — idiomatic Python implementation.
...
class InMemoryStorage:
    ...
    with a persistent backend (database, file system, etc.).
    """
    ...
    def get_version(self, name: str, version: int) -> PromptVersion | None:
        # Linear scan is fine for the expected version counts (<1000).
...
class _CacheEntry:
    ...
    expires_at: float  # time.monotonic() value; inf for version-based
...
class PromptRegistry:
    ...
    concurrent access, wrap with a lock or use a thread-safe storage backend.
    """
    ...
    def register(
        ...
        if (
            self._config.validate_template
            and "{{" in template
            and not variables
        ):
            raise ValueError(
    ...
    def resolve(
        ...
        *,
        ...
        if self._config.cache_enabled:
            # Version-based: cache forever. Alias-based: cache with TTL.
            expires_at = (
                float("inf")
