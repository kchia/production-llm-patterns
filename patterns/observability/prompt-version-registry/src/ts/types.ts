...
export interface PromptVersion {
  ...
  contentHash: string;
  config: PromptConfig;
  /** Variable names extracted from template placeholders */
  variables: string[];
  createdAt: Date;
}

/** Result of resolving a prompt (version + resolution metadata) */
...
/** Record of an alias change for audit purposes */
...
export interface RegistryConfig {
  ...
  cacheTTL?: number;
  /** Default alias to use when no version or alias is specified (default: "production") */
  defaultAlias?: string;
  /** Maximum versions to keep per prompt before archiving (default: unlimited / Infinity) */
  maxVersions?: number;
  /** Whether to validate template variable placeholders on registration (default: true) */
